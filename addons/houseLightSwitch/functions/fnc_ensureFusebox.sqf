...
if (_allPos isEqualTo []) exitWith {
	WARNING_1("Building %1 has no positions, cant place Fuse Box!", _building);
};
...
for "_dir" from 0 to 270 step 90 do { 
...

	{
		...
		if (!(isNull _obj) && {_obj isEqualTo _building}) exitWith {
			_fusebox = "Grad_Fuse_Box" createVehicle _pos;
			_fusebox setPosASL _pos;
			_fusebox setDir ((_buildingDir + _dir) + 180) mod 360;
...
_building setVariable [QGVAR(fusebox), _fusebox, true];
_fusebox setVariable [QGVAR(building), _building, true];
