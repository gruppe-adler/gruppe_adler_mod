...
_return = _return + _tab + _tab + format ["class %1 {", _type];
...
_return = _return + _breakline + _trippleTab + format ["headgear = '%1';", _loadout select 6];
...
_return = _return + _breakline + _trippleTab + _binocular;

_return = _return + _breakline + _trippleTab + format ["map = '%1';", _loadout select 9 select 0];
_return = _return + _breakline + _trippleTab + format ["compass = '%1';", _loadout select 9 select 3];
_return = _return + _breakline + _trippleTab + format ["watch = '%1';", _loadout select 9 select 4];
_return = _return + _breakline + _trippleTab + format ["gps = '%1';", _loadout select 9 select 1];
...
_return = _return + _breakline + _trippleTab + format ["nvgoggles = '%1';", _loadout select 9 select 5];

private _items = "";
//Items in Uniform
if (!(_loadout select 3 isEqualTo []) && {!(_loadout select 3 select 1 isEqualTo [])}) then {
    _return = _return + _breakline + _breakline + _trippleTab + "addItemsToUniform[] = {";
    _items = [_loadout select 3 select 1, _breakline, _tab] call FUNC(getItems);
    _return = _return + _items;
    _return = _return + _breakline + _trippleTab + "};";
};

//Items in Vest
if (!(_loadout select 4 isEqualTo []) && {!(_loadout select 4 select 1 isEqualTo [])}) then {
    _return = _return + _breakline + _breakline + _trippleTab + "addItemsToVest[] = {";
    _items = [_loadout select 4 select 1, _breakline, _tab] call FUNC(getItems);
    _return = _return + _items;
    _return = _return + _breakline + _trippleTab + "};";
};
//Items in Backpack
if (!(_loadout select 5 isEqualTo []) && {!(_loadout select 5 select 1 isEqualTo [])}) then {
    _return = _return + _breakline + _breakline + _trippleTab + "addItemsToBackpack[] = {";
    _items = [_loadout select 5 select 1, _breakline, _tab] call FUNC(getItems);
    _return = _return + _items;
    _return = _return + _breakline + _trippleTab + "};";
};
